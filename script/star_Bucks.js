(function($, window){
    ...
    var starBucks = {
          ...
          header:function(){
              ...
              // 화면너비가 960 초과이면 left:110+'%' 로 안으로 들어가게 됨(메뉴가 사라지게 (자동 일정너비 넘어가면 창 사라지게))=> 반응형 작업필요
              ...
              $(window).resize(function(){
                if($(window).width()>960){
                  ...
                  // 강제 클릭 제이쿼리 함수 괄호안에는 이벤트 사용 좋은 방법은 아님
                  if(t==false){
                    t=true //1회만 수행
              ...
              function mobileNavfn(){
                $('.mobile-container').stop().animate({left:110+'%'},300, function(){
                  $('#mobileNav').removeClass('addMobile'); //순서 중요 콜백 함수안에 넣어주기
              });
            }
              ...
              $('.mobile-container li a').on({ //모바일의 모든 메뉴
                click:function(){
                  $(this).toggleClass('addMobile');
                  $(this).next('div').slideToggle(300);
                  $('.mobile-container li a.none-sub').removeClass('addMobile'); // 논 서브에 화살표 안나오고 클릭안되개 
              ...
              $('.main-btn').on({
                  ...
                  focusin :function(){ // 접근성 탭키 키보드 접근 가능하게
                    $('.main-btn').removeClass('addCurrent');
                    $(this).addClass('addCurrent');
                    $('.sub').stop().slideUp(0);
                    $(this).next().stop().slideDown(600,'easeOutExpo');
